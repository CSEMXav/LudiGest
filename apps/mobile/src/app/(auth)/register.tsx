...
export default function RegisterScreen() {
  ...
  async function register() {
    ...
    if (!firstName || !lastName || !email || !matricule || !password || !location) {
      return Alert.alert("Erreur", "Tous les champs obligatoires sont requis, y compris la ludothèque.");
    }
    ...
    try {
      const payload = {
        ...form,
        nickname: form.nickname.trim() || email,
      };
    ...
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={s.container} keyboardShouldPersistTaps="handled">
        ...
        <TextInput style={s.input} placeholder="Email *" value={form.email}
          onChangeText={(v) => { set("email", v); if (!form.nickname) set("nickname", v); }}
          autoCapitalize="none" keyboardType="email-address" autoComplete="email" />

        <TextInput
          style={s.input}
          placeholder={`Surnom (défaut : ${form.email || "votre email"})`}
          value={form.nickname}
          onChangeText={(v) => set("nickname", v)}
          autoCapitalize="none"
        />
        <Text style={s.hint}>Le surnom est affiché dans la liste des membres.</Text>
        ...
        <View style={s.switchRow}>
          <View style={s.switchLabel}>
            <Text style={s.switchTitle}>Visible dans la liste des membres</Text>
            <Text style={s.switchDesc}>Mon surnom et mes emprunts seront visibles par les autres.</Text>
          </View>
          <Switch
            value={form.visibleInMembers}
            onValueChange={(v) => set("visibleInMembers", v)}
...
const s = StyleSheet.create({
  ...
  switchRow:             { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderRadius: 14, padding: 16, marginBottom: 16, gap: 12 },
  switchLabel:           { flex: 1 },
