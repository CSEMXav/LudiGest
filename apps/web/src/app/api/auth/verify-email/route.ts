...
export async function GET(req: NextRequest) {
  ...
  if (!record) {
    return new NextResponse(html(
      "Lien déjà utilisé",
      "Ce lien de confirmation a déjà été utilisé — votre compte est probablement déjà activé. Essayez de vous connecter à l'application.",
      true,
      "#2563eb"
...
function html(title: string, message: string, success: boolean, color?: string) {
  const c = color ?? (success ? "#16a34a" : "#C8102E");
  const icon = success ? "✅" : "❌";
